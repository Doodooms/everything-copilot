...
|-----------|----------------------------------|---------------------------------------------------|
| graphify  | python -m graphify.serve         | query_graph, get_node, get_neighbors, shortest_path |
| gitnexus  | npx gitnexus@latest mcp          | impact, context, detect_changes, rename            |

Run `graphify .` once to build `graph.json` before starting the graphify MCP server.
...
2. Run `graphify .` to build the initial knowledge graph.
3. Start the MCP servers (see `.vscode/mcp.json`).
4. Update `.github/PLAN.md` to reflect project-specific goals.
5. Invoke `memory` skill Orient phase to seed context.
...
1. Invoke `graphify` skill -- reads GRAPH_REPORT.md, surfaces god nodes.
2. Invoke `gitnexus` skill -- runs `context` on key symbols.
3. Reference `#file:graphify-out/GRAPH_REPORT.md` in chat for architecture questions.
...
| P2    | Import `code-architect` agent                    | Planned     |
| P2    | Import `code-explorer` agent                     | Planned     |
| P2    | Import `database-reviewer` agent                 | Planned     |
| P2    | Import `silent-failure-hunter` agent             | Planned     |
| P2    | Import `pr-test-analyzer` agent                  | Planned     |
| P2    | Import `e2e-runner` agent                        | Planned     |
